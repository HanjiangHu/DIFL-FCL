...
def define_G(input_nc, output_nc, ngf, n_blocks, n_blocks_shared, n_domains, norm='batch', use_dropout=False,
             gpu_ids=[]):
    ...
    if n_blocks_shared > 0:
        ...
        plex_netG = G_Plexer(n_domains, ResnetGenEncoder, enc_args, ResnetGenDecoder, dec_args, ResnetGenShared,
                             shenc_args, shdec_args)
    else:
...
class GANLoss(nn.Module):
    ...
    def __call__(self, inputs, is_real):
        ...
        multipliers = list(range(1, len(inputs) + 1));
        multipliers[-1] += 1
